( function( $ ){
    ...
    function switch_tab( tab ){
		//remove use of URL
		var previous_tab = getUrlParam( "tab" );
		var active_tab = ( tab )?tab:( previous_tab )?previous_tab:'general-settings';
		setUrlParam( 'tab', active_tab );
		
		$( '.nav-tab' ).removeClass( 'nav-tab-active' );
		$( 'a[href^="?tab='+active_tab+'"]' ).addClass( 'nav-tab-active' );
		$( '.tab' ).hide();
	...
	function setUrlParam( param_name, param_value ){
		...
		if( tr ){
			url = url.replace( tr, param_value );
		}else{
			var s = ( -1 === url.indexOf('?') )?'?':'&';
	...
	function update_referer( previous_tab, active_tab, page_settings ){
		$( '.referer-link' ).each(function( i, e ) {
			...
			if( null !== previous_tab && -1 !== href.search( '%26tab%3D'+previous_tab ) ){
